...
# BTC funding rate sources: Binance (primary), Hyperliquid (fallback)
...
def compute_rsi_for_cane() -> Optional[float]:
    ...
    try:
        ...
        if data.empty or len(data) < 14:
            ...
            return None
        # Extract Close column - yfinance returns MultiIndex DataFrame for single ticker
        close_series = data["Close"]
        if isinstance(close_series, pd.DataFrame):
            close_series = close_series.iloc[:, 0]
...
def fetch_btc_funding_rate() -> Optional[float]:
    """Fetch BTC funding rate — Binance primary, Hyperliquid fallback"""
    # Try Binance futures API (free, no auth)
    try:
        ...
        if data and len(data) > 0:
            ...
            logger.info(f"BTC funding rate from Binance: {rate}")
            return rate
    except Exception as e:
        logger.warning(f"Binance funding rate failed: {e}")

    # Fallback: Hyperliquid (free, no auth)
    try:
        ...
        if isinstance(data, list) and len(data) >= 2:
            ...
            contexts = data[1]
            # Find BTC in universe (indexed same as contexts)
            for i, asset_info in enumerate(universe):
                if asset_info.get("name") == "BTC":
                    rate = float(contexts[i].get("funding", 0))
                    logger.info(f"BTC funding rate from Hyperliquid: {rate}")
                    return rate
    except Exception as e:
