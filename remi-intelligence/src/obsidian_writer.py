...
def write_theme_note(theme_id: int, conn: sqlite3.Connection) -> Optional[str]:
    ...
    source_links = []
    sentiments = []
    for title, sname, stier, surl, facts_j, ops_j, quotes_j, tickers_j, sent, ing in rows:
        try:
...
def write_all_completed(conn: Optional[sqlite3.Connection] = None) -> dict:
    ...
    # Themes not yet written or updated since last write
    cur.execute("""
